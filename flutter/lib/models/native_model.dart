...
class PlatformFFI {
  ...
  late ItsystemsdeskImpl _ffiBind;
  ...
  ItsystemsdeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    try {
      ...
      _ffiBind = ItsystemsdeskImpl(dylib);
  ...
  void _startListenEvent(ItsystemsdeskImpl itsystemsdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = itsystemsdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
